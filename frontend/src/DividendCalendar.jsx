...
function DividendCalendar() {
  ...
  return (
    <div style={{ color: '#fff' }}>
      <h2 style={{ marginBottom: '10px' }}>📅 Kalendarz dywidend</h2>
      ...
      {error && <div style={{ color: '#FF5252', marginBottom: '20px' }}>{error}</div>}
      {loading && <div style={{ color: '#aaa', marginBottom: '20px' }}>Ładowanie danych o dywidendach...</div>}
      ...
      {summary && (
        ...
        >
          <div>
            ...
            <strong style={{ color: '#4CAF50' }}>{summary.total_realized_last_12mo_pln.toFixed(2)} PLN</strong>
          </div>
          <div>
            📈 Szacowany roczny dochód:{' '}
            <strong style={{ color: '#FFA726' }}>{summary.total_annual_estimate_pln.toFixed(2)} PLN</strong>
            <span style={{ color: '#777', fontSize: '12px' }}> (estymacja)</span>
          </div>
        </div>
      )}

      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <button onClick={() => setView('szacowane')} style={toggleTabStyle(view === 'szacowane')}>
          🔮 Szacowana dywidenda
        </button>
        <button onClick={() => setView('realne')} style={toggleTabStyle(view === 'realne')}>
          ✅ Realna dywidenda (wypłacona)
        </button>
      </div>

      {withHistory.length === 0 && !loading ? (
        <div style={{ color: '#aaa' }}>Żadna ze spółek w portfelu nie ma danych o dywidendach w Yahoo Finance.</div>
      ) : (
              ...
              <Fragment key={p.ticker}>
                <tr style={{ borderBottom: '1px solid #444' }}>
                  <td style={{ padding: '8px' }}>
                    ...
                    <div style={{ fontSize: '12px', color: '#999' }}>{p.ticker}</div>
                  </td>
                  ...
                  {view === 'szacowane' ? (
                    <>
                      ...
                      <td style={{ padding: '8px', color: '#FFA726' }}>
                ...
                {view === 'realne' && expandedTicker === p.ticker && (
                  <tr>
                    <td colSpan={5} style={{ padding: '10px 10px 10px 30px', background: '#242424' }}>
                      {p.realized_dividends.map((d, idx) => (
                        <div key={idx} style={{ fontSize: '13px', color: '#ccc', marginBottom: '4px' }}>
